...
<body class="hold-transition sidebar-mini layout-fixed">
							...
							<tbody>
								<c:forEach items="${videos}" var="item">  
									<tr>
										...
										<td>
											<img src="${item.poster}" width="250px" height="160px">
										</td>
										<td>
											<a href="<c:url value='/admin/video?action=edit&href=${item.href}' /> " type="button" class="btn btn-block btn-warning btn-sm">Edit</a>
	...
	function deleteVideo(href) {
		...
		$.ajax({
			url: '/NguyenTheHung/admin/video?action=delete&href=' + href
		}).then(function(data) {
			window.location.href = "http://localhost:8080/NguyenTheHung/admin/video?action=view";
		}).fail(function(error) {
			alert("Xin lỗi ! Vui lòng thử lại.");
